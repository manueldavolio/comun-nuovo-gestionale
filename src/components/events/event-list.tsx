    ...
    <section className="rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
      ...
      {events.length === 0 ? (
        ...
        <ul className="mt-4 space-y-3">
          {events.map((event) => {
            const showManageLinks = Boolean(attendanceBasePath || convocationBasePath);
            const showEdit = Boolean(editBasePath && event.canEdit);
            const showDelete = Boolean(eventDeleteEndpointBase && event.canDelete);
            ...
            return (
              <li key={event.id} className="rounded-lg border border-blue-100 p-3">
                ...
                {showManageLinks || showEdit || showDelete ? (
                  <>
                    {showManageLinks ? (
                      event.category ? (
                    ...
                    {showEdit || showDelete ? (
                      <div className="mt-3 flex flex-wrap items-start gap-2">
                        {showEdit ? (
                          <Link
                            href={`${editBasePath}/${event.id}/modifica`}
