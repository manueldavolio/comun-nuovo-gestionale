...
type AdminSiteStaffPageProps = {
  searchParams: Promise<{ sezione?: string }>;
};

export default async function AdminSiteStaffPage({ searchParams }: AdminSiteStaffPageProps) {
  const session = await requireSiteWebAdminPage("/admin/sito-web/staff");

  const params = await searchParams;
  const sezione = (SITE_STAFF_CATEGORIES as readonly string[]).includes(params.sezione ?? "")
    ? (params.sezione as string)
    : "all";

  const members = await prisma.siteStaffMember.findMany({
    where: sezione === "all" ? {} : { category: sezione },
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-sky-50 to-blue-100 p-4 md:p-8">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4">
        <AreaHeader
          title="Staff sito web (Admin)"
          subtitle="Staff e dirigenza mostrati sul sito pubblico"
        ...
        <section className="rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm text-zinc-600">{members.length} membro/i staff</p>
              <p className="mt-1 text-xs text-zinc-500">
                Sezione: <span className="font-semibold">{sezione === "all" ? "Tutte" : sezione}</span>
              </p>
            </div>

            <div className="flex w-full flex-col gap-2 md:w-auto md:items-end">
              <Link
                href="/admin/sito-web/staff/nuovo"
          ...
          {members.length === 0 ? (
            <p className="mt-4 rounded-lg border border-zinc-200 bg-white p-4 text-sm text-zinc-700">
              Nessun membro staff inserito. Crea il primo con &quot;Nuovo membro staff&quot;.
            </p>
          ) : (
              ...
              <table className="min-w-full divide-y divide-blue-100 text-sm">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wide text-blue-800">
                    ...
                    <th className="px-3 py-2 font-semibold">Sezione</th>
                ...
                <tbody className="divide-y divide-blue-50 text-zinc-700">
                  {members.map((member) => (
                    <tr key={member.id}>
                      ...
                      <td className="px-3 py-2">
                        <Link
                          href={`/admin/sito-web/staff/${member.id}/modifica`}
